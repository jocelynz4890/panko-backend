...
type SnapshotDoc = Awaited<
  ReturnType<SnapshotConcept["_getSnapshots"]>
>[number];
...
Deno.test("Snapshot Concept - Action Tests", async (t) => {
  ...
  try {
    await t.step("createSnapshot: enforces ranking requirements", async () => {
      ...
      console.log(
        "Trace: createSnapshot with ranking 6 returned:",
        resultInvalid,
      );
      ...
      console.log(
        "Trace: createSnapshot with ranking 4 returned:",
        resultValid,
      );
    ...
    await t.step(
      ...
      async () => {
        ...
        // Verify Effect
        const stored = await concept._getSnapshots({
          recipe: asID("recipe:2"),
        });
        ...
        assert(
          nonExistentRes.error,
          "Should fail when editing non-existent snapshot",
        );
      },
    );
    ...
    await t.step(
      ...
      async () => {
        ...
        // Verify
        const remainingA = await concept._getSnapshots({
          recipe: asID("recipe:A"),
        });
        const remainingB = await concept._getSnapshots({
          recipe: asID("recipe:B"),
        });
    ...
    await t.step("editSnapshot: allows partial updates", async () => {
      const { snapshot } = await concept.createSnapshot({
        user: TEST_USER,
        ...
        note: "Needs salt",
        ranking: 2,
        recipe: asID("recipe:partial"),
      });
      assert(snapshot);

      const partialEdit = await concept.editSnapshot({
        snapshot,
        note: "Added salt",
      });
      assert(!partialEdit.error);

      const stored = await concept._getSnapshots({
        recipe: asID("recipe:partial"),
      });
      const updated = stored.find((s: SnapshotDoc) => s._id === snapshot);
      assertEquals(updated?.note, "Added salt");
      assertEquals(updated?.instructions, "Boil");
    });
  } finally {
    await client.close();
  }
  ...
  try {
    await t.step(
      ...
      async () => {
        ...
        console.log(
          `User sees: Attempt 1 ranked ${
            history[0].ranking
          }, Attempt 2 ranked ${history[1].ranking}`,
        );
        console.log(
        ...
        const updatedAttempt2 =
          (await concept._getSnapshots({ recipe: RECIPE_ID })).find(
            (s: SnapshotDoc) => s._id === attempt2.snapshot,
          );
        assertEquals(
          updatedAttempt2?.instructions,
          "Bulk ferment 5 hours at 75F",
        );
