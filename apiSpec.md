...
# Requesting Upload Endpoint

**Purpose:** Document the standalone upload route hosted by the Requesting server (not a concept action) so clients know how to attach images to recipes before calling the JSON-only Recipe APIs.

---

### POST /api/uploads/recipe-image

**Description:** Streams a recipe image to Cloudinary, enforces limits, and records the hosted URL on the specified recipe.

**Requirements:**
- `Content-Type` must be `multipart/form-data`.
- The `recipe` field must reference an existing Recipe ID.
- The `file` field must contain a non-empty image (JPG, JPEG, PNG, WEBP, or GIF).
- The image must be ≤ the `limits.maxBytes` value (defaults to 5 MB).

**Effects:**
- Validates the upload payload and size constraints.
- Uploads the file to Cloudinary with recipe/user tagging.
- Persists the returned image URL to the Recipe via `addRecipePicture`.
- Returns the uploaded image metadata and current limit settings.

**Request Body (multipart form-data fields):**
- `recipe` (string, required): Recipe ID the image belongs to.
- `user` (string, optional): User ID for tagging/auditing.
- `file` (binary, required): Image file in an allowed format.
